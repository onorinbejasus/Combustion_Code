...
StreamlineRenderer::StreamlineRenderer(){
	this->bReadyToProcess = false ;		// We are not ready to generate StreamlineRenderers
	...
	streamlineColor[0][0] = 0.2 ; streamlineColor[0][1] = 0.3 ;	streamlineColor[0][2] = 1 ;	
	streamlineColor[1][0] = 0.3 ; streamlineColor[1][1] = 1 ;	streamlineColor[1][2] = 0.4 ;	
	streamlineColor[2][0] = 1 ; streamlineColor[2][1] = 0.3 ;	streamlineColor[2][2] = 0.2 ;	
	//unselected streamline
	streamlineColor[3][0] = 0.8 ; streamlineColor[3][1] = 0.8 ;	streamlineColor[3][2] = 0.8 ;	
	//temporary selected streamline
	streamlineColor[4][0] = 0.5 ; streamlineColor[4][1] = 0.5 ;	streamlineColor[4][2] = 0.5 ;		
} 

void StreamlineRenderer::biLinearInterpolate(Vector& v1, Vector& v2, Vector& v3, Vector& v4, float fDelta1, float fDelta2){
	v1.Interpolate(v2,fDelta1) ;
	v3.Interpolate(v4,fDelta1) ;
	v1.Interpolate(v3,fDelta2) ; 
}

void StreamlineRenderer::Configure(Vector vSeedPoint, int n){
	this->fSeedPosX = vSeedPoint.VectorX ;								// set the seed point
	this->fSeedPosY = vSeedPoint.VectorY ;
	this->fSeedPosZ = vSeedPoint.VectorZ ;
...
void StreamlineRenderer::SetSpacing(float dx, float dy, float dz){
	this->boxLenX = dx ;
	this->boxLenY = dy ;
	this->boxLenZ = dz ;
} 

void StreamlineRenderer::DrawSeedPoint(){
	glPushMatrix() ;
...
void StreamlineRenderer::DeleteStreamlines(){
...
void StreamlineRenderer::getUniquePoints(){

	// previous vector
	Vector prev(0.0, 0.0, 0.0);
	std::vector<Vector*> temp;

	for(int i = 0; i < points.size(); i++){

			if( ceil(points[i]->X()) != ceil(prev.X()) // if the point differs
				|| ceil(points[i]->Y()) != ceil(prev.Y())
					|| ceil(points[i]->Z()) != ceil(prev.Z()) )
						temp.push_back(points[i]); // store it
			
			prev = points[i]; // next
	}

	points.clear(); // clear old
	points = temp; // copy new
...
bool StreamlineRenderer::DerivativeAtPosition(Vector CurrPos, Vector& vDerivative){
...
bool StreamlineRenderer::EulerStep(Vector CurrPos, Vector& NextPos, float fStepSize){
...
bool StreamlineRenderer::MidPointMethod(Vector CurrPos, Vector& NextPos, float fStepSize){
...
bool StreamlineRenderer::RK4Method(Vector CurrPos, Vector& NextPos, float fStepSize){
...
void StreamlineRenderer::CreateStreamlines(){
	...
	for( int k = -2 ; k <= 2 ; k+=2){
		for(int i = -2 ; i <= 2 ; i+=2){
			for(int j = -2 ; j <= 2 ; j+=2){
				...
				iStepCount = 0 ;
				while( iStepCount < 20000 )			// max of 10000 steps
				{
					// do one euler step
					...
					glBegin(GL_LINES) ;											// Now, draw the line
						points.push_back(new Vector(CurrPos.X()*this->boxLenX, CurrPos.Y()*this->boxLenY,CurrPos.Z()*this->boxLenZ));
...
void StreamlineRenderer::RenderStreamlines(){
	//makeCurrent();
...
void StreamlineRenderer::SelectModeSeedPointDraw(){
	if( !this->bVisible  )
		return ;
	glPushMatrix() ;
...
void StreamlineRenderer::SelectModeStreamlinesDraw(){
...
void StreamlineRenderer::UpdateStreamlineStatus(int iName){
...
void StreamlineRenderer::TemporarySelectedStreamline( int iName){
	int iArrayIndex = iName - this->StreamlineDisplayListIndex ;
	this->TempSelectedStreamline = iArrayIndex ;
} 
void StreamlineRenderer::UpdateSeedPoint(){
	this->DeleteStreamlines() ; 
	this->Configure( Vector(this->fSeedPosX, this->fSeedPosY, this->fSeedPosZ) ,this->iNumberOfStreamlines) ;
	this->CreateStreamlines() ; 
} 


void StreamlineRenderer::GetSeedPoint(Vector& vSeed){
	//fPoint seed(this->fSeedPosX, this->fSeedPosY, this->fSeedPosZ ) ;
	vSeed.VectorX = this->fSeedPosX ;
	vSeed.VectorY = this->fSeedPosY ;
	vSeed.VectorZ = this->fSeedPosZ ;	
} 
// change the seed point location but don't update streamlines 
void StreamlineRenderer::MoveSeedPoint(int iDiff){
