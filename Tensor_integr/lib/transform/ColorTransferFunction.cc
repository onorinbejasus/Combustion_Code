...
const float** ColorTransferFunction::getPointList() const
{
	...
	for(int i = 0; i < 10; i++)
	{
		pf[i] = new float[4];
		for(int j = 0; j < 4; j++)
			pf[i][j] = pointList[i][j];
	}
	return const_cast<const float**> (pf);
}

const float* ColorTransferFunction::getPoint(int index) const
{
