...
void Projector::Initialize(int iwidth, int iheight){
	printf("start Projector::Initialize\n"); fflush(stdout);

	width = iwidth;
	height = iheight;

	Vector location(0,0,2500);
	Vector direction(0.0, 0.0, -1.0) ;
	float fSourceToDetector = Parameters::RedSourceToDetector;
	iPoint viewportSize(width,height,0);
	float fDetectorPixelSize = 0.125;
	Vector centralSpot((double)width/2,(double)height/2, 0.0) ;
    this->Configure(location,                  // set up the projector system
                    direction,
                    fSourceToDetector,
                    viewportSize,
                    fDetectorPixelSize,
                    centralSpot);
    this->iFastPassSamplingInterval = 8;				// configure the fast-pass projection method		
																// every 8th pixel is ray-casted. Rest are interpolated
	//updating the transfer functions
	//this->opacityFunction = *Parameters::opTranFunc ;
	//this->colorTranFun = *Parameters::colTranFunc ; 

	// the scaling factors
	this->vProjectorScale.fVector[0] = 1;  
	this->vProjectorScale.fVector[1] = 1;  
	this->vProjectorScale.fVector[2] = 1;  
	//calculate transformation matrices
	this->CalculateTransformations() ; 
	//projector.printProjector() ;
	printf("end Projector::Initialize\n"); fflush(stdout);
}
...
#undef FindFrustum
    ...
    vImageMinScrCoord.VectorX = std::max(vImageMinScrCoord.VectorX, 0.0f);
    vImageMinScrCoord.VectorY = std::max(vImageMinScrCoord.VectorY, 0.0f);
    vImageMaxScrCoord.VectorX = std::min(vImageMaxScrCoord.VectorX, (float)(ipImageSize.x - 1));
    vImageMaxScrCoord.VectorY = std::min(vImageMaxScrCoord.VectorY, (float)(ipImageSize.y - 1));
...
#define vRayCastStepDelta   vPixelInCtObjCoords      // we will now use a different name for the same variable
            ...
            for (int i=0; i<4; i++)
            {
                ...
                iSampleCount = std::max(iSampleCount, (int)((float*)&m128_Count)[i]);  // drop fractional part
            }
...
NoPiercing:
        ...
        if (this->bFastPassMethod)
        {
                ...
                for (int iScreen=iStart; iScreen<=iEnd; iScreen+=iStep)
            {
                ...
                int jInternalStep = std::min(jStep, ipImageSize.y - jScreen - 1);
                int iInternalStep = std::min(iStep, ipImageSize.x - iScreen - 1);
...
void Projector::NormalizeRGB(){
		...
		float globalMax= std::max(std::max(fMaxRedValue,fMaxGreenValue),fMaxBlueValue) ;
        for (int jScreen=jStart; jScreen<=jEnd; jScreen++)         
			for (int iScreen=iStart; iScreen<=iEnd; iScreen++){
...
void Projector::ChangeLocation(float fDelta){
