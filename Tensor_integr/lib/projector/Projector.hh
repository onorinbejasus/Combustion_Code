...
#define MaxFour(x) std::max(std::max(((float*)&x)[0],((float*)&x)[1]),std::max(((float*)&x)[2],((float*)&x)[3]))
...
public:
	...
	void Initialize(int iwidth, int type);
...
//	void ColorRayCast(Volume& ctvObj);
	...
	// Moves back and forth along the z-axis
	void ChangeLocation(float fDelta);
	void printProjector() ;
