...
extern void draw_streamlines(GLuint vbo, GLuint shader, int size);
...
Streamline::Streamline(){
	...
	streamlineColor[3][0] = 0.2 ; streamlineColor[3][1] = 0.2 ;	streamlineColor[3][2] = 0.2 ;	
...
void Streamline::DrawSeedPoint(){
	...
	if( this->bSeedpointSelected )
    {
        ...
        std::cout << "this->bSeedpointSelected = true" << std::endl;
    }
    else
    {
		glColor3f(0.0, 0.5, 0.5) ;
        std::cout << "this->bSeedpointSelected = false" << std::endl;
    }
...
void Streamline::CreateStreamlines(){		
	...
	shader = compileShader(vertexShader, linePixelShader);
