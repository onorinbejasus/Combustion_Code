                          ...
                          Vector&   CentralRaySpotOnScreen)
{
    ...
    this->fImage    = new float*[ipImageSize.y];

    for(int ii = 0; ii < ipImageSize.y; ii++)
        this->fImage[ii] = new float[ipImageSize.x];

            /*= (float**)AllocArray2D(ipImageSize.y,
                                                            ipImageSize.x,
                                                            sizeof(float)); */
    this->fRGBImage	= new float*[ipImageSize.y];

    for(int ii = 0; ii < ipImageSize.y; ii++)
        this->fRGBImage[ii] = new float[ipImageSize.x];


            /*= (float **)AllocArray2D(ipImageSize.y, ipImageSize.x*4, sizeof(float)) ; */
