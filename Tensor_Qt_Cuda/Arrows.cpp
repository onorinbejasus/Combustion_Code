...
void VectorFieldRenderer::SetSliceXNumber(int col)
{
    if(this->bSliceXVisible == false)
        return;
    if(col < 0)
        this->iVectorFieldSliceNumberX = 0;
    else if(col > this->iCols - 1)
        this->iVectorFieldSliceNumberX = this->iCols - 1;
    else
        this->iVectorFieldSliceNumberX = col;
}

void VectorFieldRenderer::SetSliceYNumber(int row)
{
    if(this->bSliceYVisible == false)
        return;
    if(row < 0)
        this->iVectorFieldSliceNumberY = 0;
    else if(row > this->iRows - 1)
        this->iVectorFieldSliceNumberY = this->iRows - 1;
    else
        this->iVectorFieldSliceNumberY = row;
}

void VectorFieldRenderer::SetSliceZNumber(int slice)
{
    if(this->bSliceZVisible == false)
        return;
    if(slice < 0)
        this->iVectorFieldSliceNumberZ = 0;
    else if(slice > this->iSlices - 1)
        this->iVectorFieldSliceNumberZ = this->iSlices - 1;
    else
        this->iVectorFieldSliceNumberZ = slice;
}
